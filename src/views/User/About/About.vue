    ...
    <div class="contentPage">
      <div class="row" v-if="data.length > 0">
        <div class="col-md-4" v-for="(item, index) in data" :key="index">
          <div class="item">
            <div class="leftItem">
              <img :src="'http://localhost:3000/upload/' + item.HinhSach" alt="Image" class="imageItem" />
            </div>
            <div class="rightItem">
              <div class="dataItem">
            ...
            <div class="actionItem">
              <button v-if="isLoginDG" class="btnAddToCart" @click="showModalRentBook(item)">
                Mượn sách
              </button>
      ...
      <!-- Modal rent  -->
      <a-modal style="top: 40px" v-model:open="isModalRentBook" width="800px" title="Mượn sách" @ok="handleOkRentBook"
        @cancel="handleCancelRentBook" okText="Xác nhận" cancelText="Đóng" :ok-button-props="okButtonAccess">
          ...
          <div class="contentModal row">
            ...
            <div class="rightModal col">
              <div class="groupForm" style="opacity: 0.6">
                <span>Tên sách:</span>
                <input type="text" class="inputGroup" v-model="selectedItem.TenSach" name="" disabled="true" />
              </div>
              <div class="groupForm" style="opacity: 0.6">
                <span>Đơn giá/ngày:</span>
                <input type="number" class="inputGroup" min="1" v-model="selectedItem.DonGia" name="" disabled="true" />
              </div>
              <div class="groupForm" style="opacity: 0.6">
                <span>Số quyển:</span>
                <input type="number" class="inputGroup" v-model="selectedItem.SoQuyen" name="soQuyen" disabled="true" />
              </div>
              ...
              <div class="groupForm">
                <span>Ngày mượn</span>
                <input type="date" min="1" class="inputGroup" v-model="ngayMuon" name="ngayMuon" @change="checkDate" />
              </div>
              ...
              <div class="groupForm">
                <span>Ngày trả</span>
                <input type="date" min="1" class="inputGroup" v-model="ngayTra" name="ngayTra" />
              </div>
              ...
              <div class="groupForm">
                <span>Số lượng:</span>
                <input min="1" type="number" class="inputGroup" v-model="soLuong" name="soLuong" />
              </div>
